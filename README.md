# Master's Thesis: Deep Learning in MR Lesions and Symptom Correlation in Multiple Sclerosis
...
Several of the Python scripts in the repository are created to process a subset of files. This was done in order to enable running multiple terminals at the same time, each processing a different subset. Thus, the total amount of time needed to process all files is reduced.

## Image processing

This folder consists of scripts describing the performed preprocessing of the MRI volumes before obtaining MS brain lesions. Contrast-enhanced T1-weighted volumes and T2-FLAIR volumes were cropped, bias corrected and aligned with each other. Additionally, the preprocessed volumes were structured as required by the nnU-Net algorithm.
...
This script extracts the scaled and bias corrected volumes from the FSL anat outputs. The T2-FLAIR volumes were registered to the T1-weighted volumes using FSL's FLIRT. Subsequently, the T1 volumes and registered T2-FLAIR volumes were renamed and the folders were structured to prepare for running nnU-Net inference.
...
This notebook consists of the code used to extract T1 lesions from T2 lesions through a threshold-based approach. The threshold was set as the 5th percentile of the grey matter intensities, and the tissue segmentation acquired from FSL anat was therefore used to extract grey matter. All T2 lesion voxels that had intensities lower than the threshold were classified as T1 lesion voxels. 

- **3D-UNET_T1_segmentation.ipynb:**
...
Warp fields were obtained through non-linear registration of the standard brain template to the patient's T1-weighted volumes using FSL's FNIRT. The transformation matrices from FLIRT were used as initial transformations during the registration process. 

- **apply_warp_field.py:**
FSL's applywarp was used to convert standard atlases to subject space by applying the warp fields from FNIRT. In this code, atlases of the cortical and subcortical structures were converted to subject space for each patient, although the concept is the same for any other standard atlases or templates. 
...
In order to calculate the NCI, a CSV-file containing the paths to the SMA and brainstem parcellations, in addition to the labelled lesion masks from SPACE-MS, was needed. This code creates a dataframe containing these paths and saves it as a CSV-file.
